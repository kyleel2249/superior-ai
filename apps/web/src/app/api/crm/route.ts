...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "test") return NextResponse.json(await connector.testConnection());
    if (body.action === "upsert_contact") return NextResponse.json(await connector.upsertContact(body.contact ?? {}));
    if (body.action === "create_deal") return NextResponse.json(await connector.createDeal(body.deal ?? { name: "Untitled" }));
