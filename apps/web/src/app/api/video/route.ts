...
export async function POST(req: NextRequest) {
  try {
    ...
    const storyBoard = body.storyBoard ?? buildStoryBoard({
      product, audience, region: body.region,
      painPoint: body.painPoint ?? "manual follow-ups and lost leads",
      offer: product, cta: body.cta ?? "Book a demo",
      durationSec: Number(body.durationSec ?? 30),
      style: body.style ?? "ugc", platform: body.platform ?? "tiktok",
    });
    ...
    const result = await generateVideo({
      storyBoard, platform: body.platform ?? "tiktok", style: body.style ?? "ugc",
      aspectRatio: body.aspectRatio ?? "9:16", continuity, provider: body.provider ?? "auto",
    });
    return NextResponse.json({ ...result, storyBoard, disclaimer: "mediaProduced=false means no synthetic video URLs were invented." });
  } catch (err) {
