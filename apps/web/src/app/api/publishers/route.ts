...
import {
  registerPublisher, linkPackToPublisher, recordPackSale,
  listPublishers, listRevenueEvents, publisherBalance,
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ publishers: listPublishers(), events: listRevenueEvents().slice(0, 50) });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "publish_pack") {
      if (!body.pack) return NextResponse.json({ error: "pack required" }, { status: 400 });
      const signed = publishPack(body.pack, { usePlatformSign: true });
      if (body.publisherId) linkPackToPublisher(signed.id, body.publisherId);
    ...
    if (body.action === "sign_preview") {
      ...
      return NextResponse.json({ signed, verification: verifyPackSignature(signed) });
    }
