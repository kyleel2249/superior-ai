...
test.describe("Command center flows", () => {
  ...
  test("campaign API", async ({ request }) => {
    ...
    expect(data.campaign || data.storyBoard || data.adCopy || data.package).toBeTruthy();
  });
  ...
  test("billing budget", async ({ request }) => {
    ...
    expect(res.ok()).toBeTruthy();
    const get = await request.get("/api/billing?organizationId=org_e2e");
    expect(get.ok()).toBeTruthy();
    const data = await get.json();
    expect(data.budget?.budget?.monthlyLimitUsd).toBe(50);
  });
