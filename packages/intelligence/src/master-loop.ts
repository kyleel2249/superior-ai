export const MASTER_LOOP_STAGES = [
  "remember", "understand_customer", "understand_market", "research", "analyze_competitors",
  "generate_ideas", "validate_ideas", "create_product", "create_brand", "create_software",
  "create_media", "publish", "traffic", "leads", "sales", "support", "analyze_cx",
  "optimize_ops", "optimize_costs", "retain", "collect_data", "learn", "improve", "diversify", "expand",
] as const;

export type MasterLoopStage = (typeof MASTER_LOOP_STAGES)[number];

export function planMasterLoop(objective: string) {
  return {
    id: `loop_${Date.now().toString(36)}`,
    objective,
    stages: MASTER_LOOP_STAGES.map((stage) => ({
      stage,
