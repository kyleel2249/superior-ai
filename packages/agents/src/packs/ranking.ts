...
export function recordPackInstall(packId: string): void {
  const m = metricsStore.get(packId) ?? { packId, installs: 0, ratingSum: 0, ratingCount: 0 };
  m.installs += 1;
...
export function getPackMetrics(packId: string): PackMetrics {
  return metricsStore.get(packId) ?? { packId, installs: 0, ratingSum: 0, ratingCount: 0 };
}
...
export function scorePack(pack: AgentPackManifest) {
  ...
  const ratingScore = ratingAvg == null ? 0 : (ratingAvg / 5) * (m.ratingCount >= 3 ? 20 : 8);
  ...
  const score =
    Math.round((verifiedBoost + featuredBoost + installScore + ratingScore + recencyBoost + pricingBoost) * 100) / 100;
  return {
    score,
    metrics: {
      installs: m.installs,
...
export function rankCatalog(filter?: { category?: string; organizationId?: string; limit?: number }): RankedPack[] {
  const packs = listCatalog(filter?.category ? { category: filter.category as never } : undefined);
