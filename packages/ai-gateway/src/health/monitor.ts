 ...
 * Continuously scores providers; removes unhealthy from routing pool.
 */

import type { ProviderId, ModelStatus } from "@superior-ai/core";
import { getAdapter, configureAndValidate } from "../providers";
...
export async function checkProvider(provider: ProviderId, apiKey?: string, baseUrl?: string): Promise<ProviderHealthSnapshot> {
  ...
  if (apiKey) {
    adapter.setCredentials({ apiKey, baseUrl });
  }
  ...
  // Update model availability for this provider
...
export async function checkAllFromEnv(): Promise<ProviderHealthSnapshot[]> {
  const configs: Array<{ provider: ProviderId; key?: string; base?: string }> = [
    ...
    { provider: "openrouter", key: process.env.OPENROUTER_API_KEY, base: process.env.OPENROUTER_BASE_URL || "https://openrouter.ai/api/v1" },
