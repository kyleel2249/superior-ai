/**
 * apps/web/src/app/api/knowledge/route.ts and .../chat/route.ts both import
 * ingestDocument / retrieve / buildRagContext — the original lexical-only
 * RAG API. rag.ts was later rewritten into a hybrid vector+lexical design
 * (hybridRetrieve / ingestKnowledge / ragStatus) that doesn't provide these
 * names, but neither route was updated to match, so this restores them as a
 * standalone lexical layer rather than breaking those two routes.
 */
