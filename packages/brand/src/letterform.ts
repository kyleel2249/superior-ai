/**
 * Letterform-to-icon / brand identity concepts
 * Generates structured design systems — SVG marks when possible; no fabricated raster URLs.
 */
...
export interface BrandSystem {
  ...
  industry: string;
  personality: string[];
  concepts: LetterformConcept[];
  palette: { name: string; hex: string; role: string }[];
  wordmarkNotes: string;
  faviconNotes: string;
  socialAvatarNotes: string;
  brandGuideOutline: string[];
  createdAt: string;
}
...
function svgMonogram(letters: string, style: BrandStyle): string {
  ...
  // Simple geometric mark with letterforms — deterministic SVG, not an external image URL
...
export function generateLetterformConcepts(input: {
  ...
  const personality = input.personality?.length
    ? input.personality
  ...
  const concepts: LetterformConcept[] = styles.map((style, i) => {
    ...
    return {
      ...
      typography: {
        headline: style === "luxury" ? "High-contrast serif or refined grotesk" : "Geometric sans (e.g. Inter / Satoshi family)",
