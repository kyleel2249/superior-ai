/**
 * Agent pack billing — map add_on / enterprise packs to Stripe prices
 */

import { createCheckoutSession } from "./checkout";

const DEFAULT_PRICE_MAP: Record<string, string> = {
  // Override via env PACK_PRICE_<ID> or PACK_PRICE_JSON
...
export async function checkoutAgentPack(input: {
  ...
  successUrl?: string;
  cancelUrl?: string;
}): Promise<{ url?: string; id?: string; error?: string; priceId?: string }> {
  const priceId = getPackPriceId(input.packId) || process.env.STRIPE_PRICE_ID;
  if (!priceId) {
    return {
      error: `No Stripe price for pack ${input.packId}. Set PACK_PRICE_JSON or STRIPE_PRICE_ID.`,
    };
  }
  ...
  const result = await createCheckoutSession({
    ...
    successUrl:
      input.successUrl ||
      `${appUrl}/admin/packs?checkout=success&pack=${encodeURIComponent(input.packId)}`,
...
export function listPricedPacks(): Array<{ packId: string; priceId: string }> {
