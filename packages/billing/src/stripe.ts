...
export async function reportMeteredUsage(input: {
  ...
  try {
    ...
    if (!res.ok) return { ok: false, error: `Stripe HTTP ${res.status}` };
    return { ok: true };
  } catch (err) {
...
export function handleWebhookEvent(event: WebhookEvent): {
  ...
  switch (event.type) {
    case "invoice.paid":
      return { handled: true, action: "activate_subscription" };
    case "invoice.payment_failed":
      return { handled: true, action: "payment_failed" };
    case "customer.subscription.deleted":
      return { handled: true, action: "cancel_subscription" };
    case "customer.subscription.updated":
      return { handled: true, action: "sync_subscription" };
    default:
