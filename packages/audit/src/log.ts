 ...
 * Append-only in-memory ring + optional stdout JSON lines for ship-to-SIEM.
 */
...
export function listAuditEvents(filter?: {
  ...
  if (filter?.organizationId) {
    list = list.filter((e) => e.organizationId === filter.organizationId);
  }
  if (filter?.actorId) {
    list = list.filter((e) => e.actorId === filter.actorId);
  }
  if (filter?.action) {
    list = list.filter((e) => e.action === filter.action || e.action.startsWith(filter.action!));
  }
